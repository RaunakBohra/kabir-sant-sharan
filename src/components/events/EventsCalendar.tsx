              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {isAlmostFull && !isFull && (
                      <span className="bg-amber-100 text-amber-800 text-xs px-2 py-1 rounded-full border border-amber-300 animate-pulse">
                        {spotsLeft} left!
                      </span>
                    )}
                    {isFull && (
                      <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full border border-red-300">
                        Full
                      </span>
                    )}
                  </div>
                  <h4 className="font-bold text-dark-900 mb-2 group-hover:text-teal-700 transition-colors line-clamp-2">
                    {event.title}
                  </h4>
                  ...
                  <button className="mt-3 w-full bg-teal-600 text-white py-2 rounded-lg hover:bg-teal-700 transition-colors text-sm font-medium">
                    View Details
                  </button>
                </div>
              )
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
              calendarType === 'AD'
                ...
                : 'text-dark-700 hover:bg-cream-300'
            }`}
          >
            AD
          </button>
                ...
                {events.slice(0, 2).map((event) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      onClick={() => setSelectedEvent(event)}
                      className={`text-xs p-1 rounded border cursor-pointer hover:opacity-80 transition-opacity duration-200 relative ${getTypeColor(event.type)}`}
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {event.registrationRequired && (
                          <>
                            ...
                            {isAlmostFull && !isFull && (
                              <span className="text-[10px] bg-amber-500 text-white px-1 rounded">{spotsLeft} left</span>
                            )}
          ...
          <div className="bg-cream-50 rounded-lg p-6 max-w-lg w-full mx-4 shadow-xl border border-cream-200 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="flex-1 bg-teal-600 text-white px-6 py-3 rounded-lg hover:bg-teal-700 transition-colors duration-200 font-medium flex items-center justify-center space-x-2"
              >
              ...
              <button
                onClick={() => window.location.href = `/events/${selectedEvent.slug}`}
                className="flex-1 border border-dark-300 text-dark-700 px-6 py-3 rounded-lg hover:bg-cream-200 transition-colors duration-200 font-medium"
              >
                View Details
              </button>
            </div>
